...
import { listClientTickets, createTicket } from '@/lib/expansion/tickets'
  ...
  return (
    <div>
      <div className="flex justify-between items-center mb-8">
        <h1 className="heading-xl text-2xl text-ink">Tickets</h1>
        <button type="button" onClick={() => setShowForm(!showForm)} className="btn-cta text-sm">Nuevo ticket</button>
      </div>
      ...
      {showForm && (
        <form onSubmit={handleCreate} className="rounded-2xl bg-white border p-6 mb-6 space-y-3">
          ...
          <button type="submit" className="btn-cta">Enviar ticket</button>
        </form>
      )}

      {!tickets.length ? (
        <EmptyState icon={MessageSquarePlus} title="Sin tickets" description="Abre un ticket para soporte, cambios o integraciones." />
      ) : (
